...
*/
	...
	METAL_LAYERS_INFO[0].horizontal = ((designRules.getMetal1Direction() == 'H'));
	METAL_LAYERS_INFO[1].horizontal = ((designRules.getMetal2Direction() == 'H'));
	METAL_LAYERS_INFO[2].horizontal = ((designRules.getMetal3Direction() == 'H'));
    ...
    MazeRouter* router = new MazeRouter(&connections, &designRules, vddLowerY, gndUpperY, &bbox, 2);
   
   //first try with metal 1 & 2 only
   //if it fails, try with metal 1-3
    ...
    router->getGrid()->getDims(&nHorizTiles, &nVerticTiles, &nLayers);
	for (int layer = 0; layer < nLayers; layer++)
		routGeom.mazeToGeometry_bidirectional(router->getGrid(), design, designRules, layer);
