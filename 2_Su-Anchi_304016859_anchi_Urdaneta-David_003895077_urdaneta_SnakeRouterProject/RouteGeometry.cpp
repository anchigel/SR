        ...
        ProjectDesignRules dr, int layer)
{
		...
		for(int i = 0; i < 2; i++) {
        //First loop finds vertical segments
		//Second loop finds horizontal segments
        ...
        for (int firstDim = 0; firstDim < nTilesFirstDim; firstDim++)
        {
            ...
            while (secDim < nTilesSecondDim)
            {
				...
				if(i == 0) { //If layer is vertical, create rectangle in first loop so that via/contact extensions will be vertical
					if(vertical) {
				...
				if(i == 1) {//When horizontal, check for single filled cell, without filled neighbors
					if(vertical == 0) { //If layer is not vertical, create rectangle in second loop
						if(secDim < nTilesSecondDim) { 
							if(i == 0) //right cell
								temp2 = grid->at(firstDim, secDim, layer);
							else //top cell
                    ...
                    //and not that we went out of bounds
                {
					...
					if (i == 0) //Via extensions are vertical
                    {
                    ...
                    {
                        if (i == 1) //Via extensions are horizontal
                        {
					...
					//10DBU^2 = 1nm^2
					//int area = (width+130)*height/10;
					//cout << "Current width: " << width+130 << " Current height: " << height << " Current area: " << area << endl;
		                      ...
		                      
{
                    ...
                    //and not that we went out of bounds
                {
                    ...
                    else currCell = GetCell(grid, layerInfo, firstDim, secDim - 2, layer); //should this actually be secDim - 2?
